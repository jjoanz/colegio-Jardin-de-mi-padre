...
export default async function IngresosPage() {
  const [ingresos, categorias, cuentas] = await Promise.all([
    prisma.ingreso.findMany({
      orderBy: { fecha: "desc" },
      include: { categoria: true, registradoPor: true, cuenta: true },
    }),
    prisma.categoriaIngreso.findMany({ orderBy: { nombre: "asc" } }),
    prisma.cuentaBancaria.findMany({ where: { activa: true }, orderBy: { nombre: "asc" } }),
  ]);

  const totalEsteMes = ingresos
    .filter((i) => {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-[var(--color-ink-soft)]">
        Dinero que entra al colegio fuera de las colegiaturas: donaciones, subvenciones, alquileres,
        etc. Se acredita automáticamente a la cuenta bancaria que elijas.
      </p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-[var(--color-line)] bg-white p-5">
          ...
          <p className="mt-1 font-mono text-2xl font-bold text-[var(--color-green)]">
            RD$ {totalEsteMes.toLocaleString("es-DO", { minimumFractionDigits: 2 })}
          </p>
        </div>
        <div className="rounded-2xl border border-[var(--color-line)] bg-white p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--color-ink-soft)]">
          ...
          <p className="mt-1 font-mono text-2xl font-bold text-[var(--color-green)]">
            RD$ {totalHistorico.toLocaleString("es-DO", { minimumFractionDigits: 2 })}
          </p>
        </div>
      </div>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 space-y-2">
          {categorias.map((c) => (
            <details key={c.id} className="group overflow-hidden rounded-2xl border border-[var(--color-line)] bg-white">
              ...
              <form
                action={actualizarCategoriaIngreso}
          ...
          {categorias.length === 0 && (
            <p className="rounded-2xl border border-[var(--color-line)] bg-white p-6 text-center text-sm text-[var(--color-ink-soft)]">
              Aún no hay categorías de ingreso. Creá la primera abajo (ej. Donaciones, Subvenciones,
              Alquileres).
            </p>
          )}
        </div>
        ...
        <form
          action={crearCategoriaIngreso}
          className="mt-3 grid gap-3 rounded-2xl border border-dashed border-[var(--color-line)] bg-white p-4 md:grid-cols-2"
        >
          <input name="nombre" placeholder="Nombre (ej. Donaciones)" required className={inputClass} />
      ...
      <section className="mt-8">
        ...
        {cuentas.length === 0 ? (
          <p className="mt-3 rounded-2xl border border-[var(--color-line)] bg-white p-6 text-sm text-[var(--color-ink-soft)]">
            Necesitás crear al menos una cuenta bancaria activa antes de poder registrar un ingreso.
          ...
          >
            ...
            <label className="text-xs text-[var(--color-ink-soft)]">
              Cuenta bancaria (aquí entra el dinero)
            ...
            <label className="text-xs text-[var(--color-ink-soft)]">
              Fuente / de quién viene
              <input name="fuente" placeholder="Ej. Fundación XYZ" className={inputClass} />
            </label>
            <label className="text-xs text-[var(--color-ink-soft)]">
              # de comprobante
            ...
            >
              Registrar ingreso (acredita la cuenta automáticamente)
            </BotonGuardar>
          </form>
        )}
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 space-y-2">
          {ingresos.map((i) => (
            <details key={i.id} className="group overflow-hidden rounded-2xl border border-[var(--color-line)] bg-white">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3">
                <div>
                  <p className="font-semibold text-[var(--color-ink)]">{i.descripcion}</p>
                  <p className="text-xs text-[var(--color-ink-soft)]">
                    {i.categoria.nombre} · {i.cuenta.nombre} · {i.fecha.toLocaleDateString("es-DO")}
                    {i.fuente && ` · ${i.fuente}`}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono text-sm font-bold text-[var(--color-green)]">
                    RD$ {Number(i.monto).toLocaleString("es-DO", { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </summary>
              ...
              <form
                ...
                className="grid gap-3 border-t border-[var(--color-line)] bg-[var(--color-paper-dark)] p-4 md:grid-cols-2"
              >
                <input type="hidden" name="ingresoId" value={i.id} />
                <p className="text-xs text-[var(--color-ink-soft)] md:col-span-2">
                  Monto, cuenta y fecha no se pueden editar porque ya generaron un movimiento
                  bancario real. Para corregirlos, anulá este ingreso y creá uno nuevo.
                </p>
                <fieldset disabled={i.estado === "ANULADO"} className="contents disabled:opacity-60">
                  <label className="text-xs text-[var(--color-ink-soft)] md:col-span-2">
                    Descripción
                    <input name="descripcion" defaultValue={i.descripcion} className={inputClass} />
                  </label>
                  <label className="text-xs text-[var(--color-ink-soft)]">
                    Fuente
                    <input name="fuente" defaultValue={i.fuente ?? ""} className={inputClass} />
                  </label>
                  <label className="text-xs text-[var(--color-ink-soft)]">
                    # de comprobante
                    <input name="numeroComprobante" defaultValue={i.numeroComprobante ?? ""} className={inputClass} />
                  </label>
                  <label className="text-xs text-[var(--color-ink-soft)] md:col-span-2">
                    Notas
                    <textarea name="notas" defaultValue={i.notas ?? ""} rows={2} className={inputClass} />
                  </label>
                  <BotonGuardar className="rounded-lg bg-[var(--color-green)] py-2 text-xs font-bold text-white disabled:opacity-60 md:col-span-2">
                    Guardar cambios
                  </BotonGuardar>
              ...
              {i.estado !== "ANULADO" && (
                <form action={anularIngreso} className="border-t border-[var(--color-line)] p-4">
                  <input type="hidden" name="ingresoId" value={i.id} />
                  <BotonGuardar
                  ...
                  >
                    Anular ingreso (retira el dinero de la cuenta)
                  </BotonGuardar>
                </form>
              )}
            </details>
